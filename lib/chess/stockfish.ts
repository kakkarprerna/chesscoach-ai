...
function createWorker(): Worker {
  ...
  engine.addEventListener(
    ...
    (event) => {
      console.error(
        "[Stockfish] Worker message error:",
        event
      );
    }
  );
      ...
      try {
        ...
        reject(
          error instanceof Error
            ? error
            : new Error(
                "Unable to communicate with Stockfish."
              )
        );
      }
    }
...
export async function analyzePosition(
  ...
  return new Promise<EngineEvaluation>(
    (resolve, reject) => {
      let evaluation = 0;
      ...
      function handleMessage(
        ...
        if (
          ...
          /*
           * Stockfish emits the principal variation
           * on the same info line:
           *
           * pv e2e4 e7e5 g1f3 ...
           */
          const pvMatch =
        ...
        if (line.startsWith("bestmove")) {
          ...
          /*
           * If the engine didn't emit a usable
           * PV, at least preserve the best move.
           */
          if (
      ...
      try {
        ...
        reject(
          error instanceof Error
            ? error
            : new Error(
                "Unable to send commands to Stockfish."
              )
        );
      }
    }
